<?php
	...
	if(isset($_GET['id'])){
		...
		$sel_email = "select * from memberRegistry where eMail='$user_email'";
		$run_email = mysqli_query($con,$sel_email);
		$check_email = mysqli_num_rows($run_email);

		if($check_email<1){
			$insertInto = "insert into memberRegistry (firstName,familyName,passWord,confirmPassword,eMail,aGe,genDer,birthDay,registerDate) values ('$first_name','$family_name','$user_password','$user_confirmPassword','$user_email','$user_age','$user_gender','$user_birthday','$regDate')";
			$run_insertInto = mysqli_query($con,$insertInto);

			$delete = "delete from adminRegister where user_id='$edit_id'";
